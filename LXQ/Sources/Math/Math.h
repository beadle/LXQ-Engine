...
namespace LXQ {
	namespace Math {
		...
		struct Vector2 : public XMFLOAT2
		{
			...
			static void SmoothStep(const Vector2& v1, const Vector2& v2, float t,
				Vector2& result);
			...
			static void Barycentric(const Vector2& v1, const Vector2& v2,
				const Vector2& v3, float f, float g, Vector2& result);
			static Vector2 Barycentric(const Vector2& v1, const Vector2& v2,
				const Vector2& v3, float f, float g);
			...
			static void TransformNormal(const Vector2& v, const Matrix& m,
				Vector2& result);
		...
		struct Vector3 : public XMFLOAT3
		{
			...
			static void SmoothStep(const Vector3& v1, const Vector3& v2, float t,
				Vector3& result);
			...
			static void Barycentric(const Vector3& v1, const Vector3& v2,
				const Vector3& v3, float f, float g, Vector3& result);
			...
			static void TransformNormal(const Vector3& v, const Matrix& m,
				Vector3& result);
		...
		struct Vector4 : public XMFLOAT4
		{
			...
			Vector4(Vector3 v3, float _w)
				: XMFLOAT4(v3.x, v3.y, v3.z, _w)
			{
			}
			...
			static void SmoothStep(const Vector4& v1, const Vector4& v2, float t,
				Vector4& result);
			...
			static void Barycentric(const Vector4& v1, const Vector4& v2,
				const Vector4& v3, float f, float g, Vector4& result);
			static Vector4 Barycentric(const Vector4& v1, const Vector4& v2,
				const Vector4& v3, float f, float g);
		...
		struct Matrix : public XMFLOAT4X4
		{
			...
			explicit Matrix(const Vector3& r0, const Vector3& r1, const Vector3& r2)
				: XMFLOAT4X4(r0.x, r0.y, r0.z, 0, r1.x, r1.y, r1.z, 0, r2.x, r2.y, r2.z, 0,
				0, 0, 0, 1.f)
			{
			}
			explicit Matrix(const Vector4& r0, const Vector4& r1, const Vector4& r2,
				const Vector4& r3)
				: XMFLOAT4X4(r0.x, r0.y, r0.z, r0.w, r1.x, r1.y, r1.z, r1.w, r2.x, r2.y,
				r2.z, r2.w, r3.x, r3.y, r3.z, r3.w)
			{
			}
			...
			static Matrix CreateBillboard(
				...
				const Vector3& cameraUp, _In_opt_ const Vector3* cameraForward = nullptr);

			static Matrix CreateConstrainedBillboard(
				const Vector3& object, const Vector3& cameraPosition,
				const Vector3& rotateAxis, _In_opt_ const Vector3* cameraForward = nullptr,
			...
			static Matrix CreatePerspectiveFieldOfView(float fov, float aspectRatio,
				float nearPlane, float farPlane);
			static Matrix CreatePerspective(float width, float height, float nearPlane,
				float farPlane);
			...
			static Matrix CreateLookAt(const Vector3& camera_pos,
				const Vector3& target_pos, const Vector3& up_vec);
			static Matrix CreateLookTo(const Vector3& camera_pos,
				const Vector3& target_vec, const Vector3& up_vec);
		...
		struct Quaternion : public XMFLOAT4
		{
			...
			float GetPitch() { return (float)-asin(2.f * z * y + 2.f * x * w); }
			float GetRoll()
			{
				float x2 = x * x;
				float z2 = z * z;
		...
		public:
			...
			Viewport(float ix, float iy, float iw, float ih, float iminz = 0.f,
				float imaxz = 1.f)
				: x(ix)
				, y(iy)
				, width(iw)
			...
			static RECT __cdecl ComputeDisplayArea(DXGI_SCALING scaling,
				...
				int outputHeight);
			static RECT __cdecl ComputeTitleSafeArea(UINT backBufferWidth,
...
namespace std {
	...
	template <>
	struct less<LXQ::Math::Vector2>
	{
		bool operator()(const LXQ::Math::Vector2& V1,
			const LXQ::Math::Vector2& V2) const
		{
	...
	template <>
	struct less<LXQ::Math::Vector3>
	{
		bool operator()(const LXQ::Math::Vector3& V1,
			const LXQ::Math::Vector3& V2) const
		{
			return ((V1.x < V2.x) || ((V1.x == V2.x) && (V1.y < V2.y)) ||
	...
	template <>
	struct less<LXQ::Math::Vector4>
	{
		bool operator()(const LXQ::Math::Vector4& V1,
			const LXQ::Math::Vector4& V2) const
		{
			return (
	...
	template <>
	struct less<LXQ::Math::Matrix>
	{
		bool operator()(const LXQ::Math::Matrix& M1,
			const LXQ::Math::Matrix& M2) const
		{
			if (M1._11 != M2._11)
	...
	template <>
	struct less<LXQ::Math::Plane>
	{
		bool operator()(const LXQ::Math::Plane& P1,
			const LXQ::Math::Plane& P2) const
		{
			return (
	...
	template <>
	struct less<LXQ::Math::Quaternion>
	{
		bool operator()(const LXQ::Math::Quaternion& Q1,
			const LXQ::Math::Quaternion& Q2) const
		{
			return (
	...
	template <>
	struct less<LXQ::Math::Color>
	{
		bool operator()(const LXQ::Math::Color& C1,
			const LXQ::Math::Color& C2) const
		{
			return (
	...
	template <>
	struct less<LXQ::Math::Ray>
	{
		bool operator()(const LXQ::Math::Ray& R1, const LXQ::Math::Ray& R2) const
		{
	...
	template <>
	struct less<LXQ::Math::Viewport>
	{
		bool operator()(const LXQ::Math::Viewport& vp1,
			const LXQ::Math::Viewport& vp2) const
		{
			if (vp1.x != vp2.x)
				return (vp1.x < vp2.x);
			if (vp1.y != vp2.y)
